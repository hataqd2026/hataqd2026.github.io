...
function optimizeSplitBox() {
    ...
    rows.forEach(row => {
        ...
        // Flatten list by individual quantity to make bin-packing simpler
        for (let i = 0; i < qty; i++) {
            items.push({
                name: name,
                category: category,
    ...
    // Max package value limit set to 1000 RMB (approx 128 EUR) to fit Chinese Customs tax-free/easy clearance lines.
    // Also, single item rule: if a single item is already > 1000 RMB, it cannot be divided, but must be in its own package.
    ...
    items.forEach(item => {
        ...
        for (let pkg of packages) {
            ...
            // Avoid mixing luxury items with food/supplements to prevent higher tax rates if possible,
            // and enforce the 1000 RMB personal baggage/postal article limit.
        ...
        // If it cannot fit anywhere, create a new package
        if (!placed) {
            packages.push({
    ...
    let totalWeightEst = 0; // rough estimation: 0.3kg per item
    
    packages.forEach(pkg => {
        const pkgValCNY = pkg.items.reduce((sum, it) => sum + it.priceCNY, 0);
        const pkgValEUR = pkg.items.reduce((sum, it) => sum + it.priceEUR, 0);
        totalWeightEst += pkg.items.length * 0.4 + 0.3; // 0.4kg per item + box weight 0.3kg
        
        // Calculate tax for this package
        // personal package duty rule: if tax is <= 50 RMB, it's waived (免征额)
        let packageTaxCNY = 0;
        ...
        pkgCard.innerHTML = `
            ...
            <div class="package-footer">
                <span>预估重量: ~${(pkg.items.length * 0.4 + 0.3).toFixed(1)} kg</span>
...
function calculateEuropeToChina(totalWeight, totalTaxEUR, packageCount) {
    ...
    EUROPE_TO_CHINA_CARRIERS.forEach(carrier => {
        ...
        // Adjust pricing based on region factor
        const adjustedBase = carrier.basePrice * region.priceFactor;
        const adjustedStep = carrier.stepPrice * region.priceFactor;
        
        if (carrier.type === "华人快递" || carrier.type === "空运特快") {
            // Price is calculated per package because they have to be split in customs
            // We assume weight is distributed across packages
            ...
            // Total logistics price for all packages
            finalPriceEUR = pricePerPkg * packageCount;
            
            // Add custom import duties? "华人快递" is often tax-inclusive or tax-guaranteed for supplements/milk powder
            if (carrier.type === "华人快递") {
                // Tax-inclusive (包税)
                finalPriceEUR += 0; // Tax is already in the freight rate
            } else {
                // Recipient pays tax
                finalPriceEUR += totalTaxEUR;
            }
        } else {
            // Official postal service: typically ships one large package, but user takes the risk of full tax and inspection.
            ...
            // Official post is never tax-inclusive; recipient pays tax on arrival
            finalPriceEUR += totalTaxEUR;
        }
        
        const finalPriceRMB = finalPriceEUR * EUR_TO_CNY;
        
        // Adjust days range based on region
        const dayRange = carrier.days.split('-');
        const minDays = parseInt(dayRange[0]) + region.daysOffset;
        const maxDays = parseInt(dayRange[1]) + region.daysOffset;
        const adjustedDays = `${minDays}-${maxDays}`;
        ...
        generatedRoutes.push({
            ...
            priceEUR: finalPriceEUR,
            days: adjustedDays,
            safety: carrier.type === "官方邮政" ? 85 : carrier.safety, // Post has lower customs safety without split box
...
function handleIDUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        uploadIDImg.onload = function() {
            // Show canvas and watermark panel
...
function drawWatermark() {
    ...
    ctx.fillStyle = "rgba(239, 68, 68, 0.28)"; // Semi-transparent red watermark
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    
    // Draw rotated text across multiple points
    ctx.save();
...
function renderRoutesList() {
    ...
    generatedRoutes.forEach(route => {
        ...
        // Apply special border classes based on tag
...
function sortRoutes(criteria) {
    ...
    if (criteria === 'price') {
        ...
        // Parse min days out of range "7-12" to sort
